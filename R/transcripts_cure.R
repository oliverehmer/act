...
#' - Transcript length zero: If \code{@length.sec} is missing or \code{<= 0} it will be set to the largest annotation endsec.
#' - Zero-duration annotations: In IntervalTiers, annotations with \code{startsec == endsec} will be merged (if multiple at same time), then extended to the next annotation, or deleted if no room is available.
...
#' @param transcriptLengthZero Logical; If \code{TRUE} a transcript with \code{@length.sec <= 0} will be corrected.
#' @param annotationsZeroDuration Logical; If \code{TRUE} zero-duration annotations in IntervalTiers will be cured.
...
transcripts_cure <- function (x,
	...
	transcriptLengthZero.corrected.count      <- 0
	...
	for (i in filterTranscriptNames) {
		x@transcripts[[i]] <- act::transcripts_cure_single(x@transcripts[[i]],
												   ...
												   transcriptLengthZero      = transcriptLengthZero,
												   annotationsZeroDuration   = annotationsZeroDuration,
												   tiersMissing              = tiersMissing,
												   warning                   = warning)
		...
		if (h$annotationsTimesReversed.deleted.count>0
			...
			if (isTRUE(h$transcriptLengthZero.corrected)) {
				transcriptLengthZero.corrected.count  <- transcriptLengthZero.corrected.count + 1L
			}
			annotationsZeroDuration.merged.count      <- annotationsZeroDuration.merged.count     + h$annotationsZeroDuration.merged.count
			annotationsZeroDuration.extended.count    <- annotationsZeroDuration.extended.count   + h$annotationsZeroDuration.extended.count
			annotationsZeroDuration.deleted.count     <- annotationsZeroDuration.deleted.count    + h$annotationsZeroDuration.deleted.count
	...
	x@history[[length(x@history)+1]] <- list(
		...
		transcriptLengthZero.corrected.count          = transcriptLengthZero.corrected.count,
